...
4.  优点：灵活性高，可自定义训练过程中的每一步细节，便于理解底层逻辑；
5.  缺点：代码量较多，需手动处理日志、模型保存等辅助功能。

#### 核心运行步骤
1.  数据加载与预处理（CSV读取→标签编码→数据集拆分）；
2.  加载 BERT 分词器与分类模型，迁移到指定设备（MPS/CUDA/CPU）；
3.  文本编码与数据封装，构建自定义数据加载逻辑；
4.  手动循环训练 epoch，每轮训练后执行验证；
5.  保存模型权重与 LabelEncoder，推理时加载并预测。
...
4.  优点：代码简洁、高效，内置学习率预热、权重衰减等优化，运行稳定；
5.  缺点：灵活性稍低，自定义训练流程需依赖框架扩展接口。

#### 核心运行步骤
1.  数据加载与预处理（与方式1一致，保证数据格式统一）；
2.  加载 BERT 分词器与分类模型，迁移到指定设备；
3.  文本编码后封装为 Hugging Face `Dataset` 对象，适配 `Trainer` 输入；
4.  配置 `TrainingArguments` 训练参数，实例化 `Trainer`；
5.  调用 `trainer.train()` 自动完成训练与评估，自动保存最佳模型；
6.  加载保存的模型与 LabelEncoder，执行推理预测。
...
## 模型测试效果
对 12 个类别各 1 条典型样本进行测试，整体准确率 91.7%（11/12），方法一测试结果如下：
![模型测试结果](bert.jpg)
方法二测试结果如下：
