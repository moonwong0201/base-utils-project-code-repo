...
- 同步实现下，模型推理阻塞主线程，多请求排队等待，并发能力极差
  - **方案**：用 `ThreadPoolExecutor` 将 CPU 密集型模型推理放到后台线程，主线成只处理网络网络 IO，避免阻塞事件循环
- 设置了双库重置机制，支持一键重置和索引重建，避免孤立向量累计，保障系统的稳定性；
- Milvus 向量索引优化：创建 IVF_FLAT 索引，调整 nlist=2048，搜索时设置 nprobe=16，优化了近似搜索速度。
...
- 数据量增至 5,000 条时，QPS 提升到了在 118 左右，并发能力提升2.6倍，系统平均响应时间约 109ms，P95延迟 220ms，说明优化架构和索引参数有效支撑了系统高并发。
