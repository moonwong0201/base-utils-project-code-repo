...
- 在意图路由层，初期用FAISS做向量相似度匹配获取用户提问的意图类型，但阈值很难调，路由命中率只有70%。基于业务数据微调了BERT模型，测试集准确率94%+，迁移到线上后路由命中率达到100%，而且不需要任何阈值调优；
- BERT编码和FAISS检索都是CPU密集型，Python异步解决不了GIL问题。用 ThreadPoolExecutor 把编码放到线程池，避免阻塞主线程。
...
| ------------------ | -------- | ---------- | ---------- | ---------- |
| 100%缓存命中       | **2390** | **19.8ms** | 100%       | 100%       |
| 70%缓存命中        | 62       | 0.68s      | 85%        | 70%        |
| 0%缓存命中(冷启动) | 8.3      | 3.4s       | 0%         | 0%         |

微调BERT场景下：

| 场景               | QPS      | 平均延迟   | 缓存命中率 | 路由命中率 |
| ------------------ | -------- | ---------- | ---------- | ---------- |
| 100%缓存命中       | **1966** | **23.9ms** | 100%       | 100%       |
| 70%缓存命中        | 59.4     | 0.76s      | 83.7%      | 80.3%       |
| 0%缓存命中(冷启动) | 5.3      | 5.67s      | 0%         | 66.7%       |

### 关键结论

**缓存命中 vs 未命中性能对比**： 

- QPS提升：**400倍**（1966 vs 5） 
- 延迟降低：**250倍**（24ms vs 6s） 
