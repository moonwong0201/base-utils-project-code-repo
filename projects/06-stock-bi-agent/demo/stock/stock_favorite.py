...
# """
# 自选股管理页面，核心功能是「可视化完成自选股的增删改查 + 清空操作」——
# 对接后端 /v1/stock/*_fav_stock 系列接口，实现 “添加自选股→查看列表→删除单只→清空全部”
# 的完整自选股管理闭环，是股票模块的核心用户个性化功能
# """

# -------------------- API 配置 --------------------
BASE_URL = "http://127.0.0.1:8000"
LIST_ENDPOINT = "/v1/stock/list_fav_stock"  # 查询自选股
ADD_ENDPOINT = "/v1/stock/add_fav_stock"    # 添加自选股
DEL_ENDPOINT = "/v1/stock/del_fav_stock"    # 删除单只自选股
CLEAR_ENDPOINT = "/v1/stock/clear_fav_stock"  # 清空自选股
...
def _call_api(endpoint: str, data: dict = None):
    """
...
def _fetch_favorites(username: str):
    ...
    return _call_api(LIST_ENDPOINT, data=data)


# 2. 添加自选股
def _add_favorite(username: str, stock_code: str):
    """添加收藏股票"""
    data = {"user_name": username, "stock_code": stock_code}
    return _call_api(ADD_ENDPOINT, data=data)
...
def _delete_favorite(username: str, stock_code: str):
    """删除收藏股票"""
    data = {"user_name": username, "stock_code": stock_code}
    return _call_api(DEL_ENDPOINT, data=data)
...
def _clear_favorites(username: str):
    """清空所有收藏股票"""
    data = {"user_name": username}
    return _call_api(CLEAR_ENDPOINT, data=data)
