...
- 多场景适配：针对股票分析 / 其余场景定制化 AI 提示词，保证回复专业性。
...
初始版本采用单 Agent 架构，所有工具集中管理。随着工具数量增加，可能会面临这些问题：

- 提示词会越写越多，领域知识混杂
- 工具调用错误
- 扩展困难，新增了领域需要修改提示词

所以采用 Multi-Agent 架构，增加三个智能体：专门处理股票领域的Agent、专门处理其他领域的 Agent 以及一个调度 Agent，流程是：

**用户输入 → Triage Agent（意图识别）→ Specialist Agent（股票/资讯/闲聊）**

| 改进点       | 实现方式                            | 效果               |
| :----------- | :---------------------------------- | :----------------- |
| **意图路由** | Triage Agent 自动分派               | /                  |
| **职责分离** | 三个 Agent 独立（chat/stock/other） | /                  |
| **工具隔离** | Specialist 只绑定相关工具           | 误调用率下降       |
| **共享连接** | 统一 MCP Server，手动 connect       | 解决连接初始化问题 |

做了小规模测试，20 条意图识别测试准确率 90%，20 次工具调用成功率 95%。
