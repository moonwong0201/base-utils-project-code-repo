...
from typing import Annotated  # 用于为函数参数添加类型注解和描述（MCP工具参数说明）
import sympy as sp  # 符号计算库，处理微分方程、积分、符号表达式解析
from sympy import symbols, Function, Eq, dsolve, simplify, lambdify, solve
from scipy.constants import R  # 导入气体常数R（用于阿伦尼乌斯方程等化工计算）
import numpy as np
import math
import random
...
@mcp.tool  # 装饰器：将普通函数注册为MCP服务的可调用工具
